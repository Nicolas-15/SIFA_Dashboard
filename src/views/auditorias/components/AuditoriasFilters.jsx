...
export function AuditoriasFilters({
  ...
  // Extraer usuarios únicos desde las auditorías cargadas
  useEffect(() => {
    if (!audits || audits.length === 0) return;
    const seen = new Map();
    audits.forEach((audit) => {
      const value = audit.email_usuario;
  ...
  const handleUserBlur = useCallback(() => {
    ...
    blurTimerRef.current = setTimeout(() => {
      ...
      if (current !== userFilterRef.current) {
        const match = allUsers.find((u) => u.email === current);
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {/* Fila 2: Filtros de fecha + usuario */}
